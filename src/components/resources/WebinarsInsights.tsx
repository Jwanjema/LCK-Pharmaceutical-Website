...
interface WebinarsInsightsProps {
  onNavigate: (page: string) => void;
}

export function WebinarsInsights({ onNavigate }: WebinarsInsightsProps) {
  const webinars = [
    {
      type: 'Live Webinar',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {insights.map((insight, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                whileHover={{ x: 5 }}
                className="bg-white rounded-xl p-4 md:p-6 shadow-md hover:shadow-xl transition-all border-l-4 border-[#339966] cursor-pointer"
              >
